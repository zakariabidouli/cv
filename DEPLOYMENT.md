...
- `API_KEY` - Must match the `API_KEY` in Railway (same value!)
  - This is used for admin login password AND backend authentication
  - User enters this value to login as admin
...
- [ ] Set `API_KEY` in Vercel (frontend) - **same value** (used for admin login)
